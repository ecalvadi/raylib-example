...
int main(void) {
    ...
    while (!WindowShouldClose()) //Detect window close button or ESC key
    {
        ...
        BeginDrawing();
                ...
                for( int i = 0; i < MAX_PARTICLES; i++ )
                {
                    if( mouseTail[i].active )
                    {
                        DrawTexturePro(smoke, (Rectangle){ 0.0f, 0.0f, (float)smoke.width, (float)smoke.height },
                                       (Rectangle){ mouseTail[i].position.x, mouseTail[i].position.y, (float)smoke.width * mouseTail[i].size, (float)smoke.height * mouseTail[i].size },
            ...
            DrawText("Press [SPACEBAR] to change the blending", 380, 20, 20, BLACK);
